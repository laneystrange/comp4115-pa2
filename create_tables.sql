...
CREATE TABLE matches (
  ...
  `matchFour` enum('one','two','null') DEFAULT NULL,
  `matchFive` enum('one','two','null') DEFAULT NULL,
  `winningTeam` varchar(50) DEFAULT NULL,
  PRIMARY KEY (`teamOne`,`teamTwo`,`event`),
  CONSTRAINT `matches_ibfk_1` FOREIGN KEY (`teamOne`) REFERENCES `team` (`name`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `matches_ibfk_2` FOREIGN KEY (`teamTwo`) REFERENCES `team` (`name`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `matches_ibfk_3` FOREIGN KEY (`event`) REFERENCES `event` (`title`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=latin1;